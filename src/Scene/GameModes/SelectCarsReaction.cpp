...
SelectCarsReaction::SelectCarsReaction(const int pNumberOfFrames,const int pSequence) : ColorChangeReaction(pNumberOfFrames,
                                                                                                 pSequence) {
...
void SelectCarsReaction::processClicks(const int x,const int y) {
...
void SelectCarsReaction::drawPlayerMissedClick(const int x,const int y) {
    ...
    for(KittiObject obj: currentFrame.getObjectsOfType(Labeltypes::CAR)){
        const GTBoundingBox box = obj.getLabel().getBoundingBox();
        drawHandler.drawDistToCorrectBox(x, y, box);
    }
    currentFrame.colorObjectsOfType(Labeltypes::CAR, Constants::RED);
}

void SelectCarsReaction::drawPlayerClickedCorrect(const int x,const int y) {
    Frame &currentFrame = this->frames.front();
    ...
    const std::vector<KittiObject> &selectedObjs = this->getClickedObjects(x, y);
